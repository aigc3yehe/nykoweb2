...
import { workflowDetailAtom, fetchWorkflowDetailAtom } from '../store/workflowDetailStore'
import { workflowEditAtom, initEditFormAtom, updateEditFormAtom, updateWorkflowAtom } from '../store/workflowEditStore'
...
const WorkflowEdit: React.FC = () => {
  ...
  const [workflowEditState] = useAtom(workflowEditAtom)
  ...
  const [, updateWorkflow] = useAtom(updateWorkflowAtom)
  ...
  // 获取工作流数据
  useEffect(() => {
    if (id && !workflowDetailState.workflow) {
      // 如果没有工作流数据，主动获取
      fetchWorkflowDetail({ workflowId: parseInt(id), refresh: false })
    }
  }, [id, workflowDetailState.workflow, fetchWorkflowDetail])

  // 初始化表单数据
  useEffect(() => {
    if (workflowDetailState.workflow) {
      const workflow = workflowDetailState.workflow

      // 初始化编辑表单
      initEditForm(workflow)

      // 设置封面图片URL - 从API数据中获取
      if (workflow.cover) {
        setCoverUrl(workflow.cover)
        // 同时更新表单中的cover字段
        updateEditForm({ cover: workflow.cover })
        // 设置本地cover状态，这样图片就能正确显示
        setCover(new File([], 'cover.jpg')) // 创建一个虚拟文件对象
      }

      // 设置参考图片URL - 从API数据中获取第一张图片
      if (workflow.reference_images && workflow.reference_images.length > 0) {
        const firstRefImage = workflow.reference_images[0]
        setRefImageUrl(firstRefImage)
        // 同时更新表单中的referenceImages字段
        updateEditForm({ referenceImages: workflow.reference_images })
        // 设置本地refImage状态，这样图片就能正确显示
        setRefImage(new File([], 'ref.jpg')) // 创建一个虚拟文件对象
      }
    }
  }, [workflowDetailState.workflow, initEditForm, updateEditForm])

  // 检查用户权限
  useEffect(() => {
    if (workflowDetailState.workflow && userState.user) {
      const currentUserDid = userState.user.tokens.did
      const workflowAuthorDid = workflowDetailState.workflow.user?.did

      if (currentUserDid !== workflowAuthorDid && userState.userDetails?.role !== 'admin') {
        // 用户没有权限编辑此工作流，重定向到详情页
        navigate(withLangPrefix(lang, `/workflow/${id}`))
        addToast({
  ...
  }, [workflowDetailState.workflow, userState.user, navigate, lang, id, addToast])
  ...
  const handleRefImageUpload = async (file: File) => {
    ...
    try {
      ...
      const uploadedUrl = await uploadFileToS3(file)
      updateEditForm({ referenceImages: [uploadedUrl] })
    } catch (error) {
  ...
  const handleRemoveRefImage = () => {
    setRefImage(null)
    setRefImageUrl(null)
    // 重要：将表单中的referenceImages字段设置为空数组，这样在保存时能检测到变化
    updateEditForm({ referenceImages: [] })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!success) {
        addToast({
          message: workflowEditState.updateError || 'Failed to update workflow',
          type: 'error'
        })
      }
    } catch (error) {
  ...
  if (workflowDetailState.isLoading || !workflowDetailState.workflow) {
    return (
      <div className="flex items-center justify-center h-full w-full p-8">
        <div className="text-center">
      ...
      <div className="md:w-[22.5rem] md:max-w-[22.5rem] mx-auto md:border md:border-line-subtle dark:border-line-subtle-dark rounded-xl bg-secondary dark:bg-secondary-dark p-6 flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            value={workflowEditState.form.name}
        ...
        <div className="flex flex-col gap-1">
          ...
          <textarea
            ...
            value={workflowEditState.form.description}
        ...
        <div className="flex flex-col gap-1">
          ...
          {/* 图片上传组件 */}
          {(cover || workflowEditState.form.cover) && coverUrl ? (
        ...
        <div className="flex flex-col gap-1">
          ...
          <textarea
            ...
            value={workflowEditState.form.prompt}
        ...
        <div className="flex flex-col gap-1">
          {/* 标题 */}
          <span className="font-switzer font-normal text-sm leading-5 align-middle text-text-main dark:text-text-main-dark">Reference Image (Optional)</span>
          {/* 图片上传组件 */}
          {(refImage || (workflowEditState.form.referenceImages && workflowEditState.form.referenceImages.length > 0)) && refImageUrl ? (
            <div className="relative w-[8.125rem] h-[8.125rem] gap-1 rounded-[0.625rem]">
              <img
                src={refImageUrl}
                alt="ref"
