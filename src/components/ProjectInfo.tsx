...
const ProjectInfo: React.FC<ProjectInfoProps> = ({ projectInfo, isLoading }) => {
  ...
  // 检查是否有有效的twitter数据
  const hasValidTwitterData = projectInfo.twitter && 
    (projectInfo.twitter.username || projectInfo.twitter.name);
  ...
  // 如果没有任何有效数据，则不渲染组件，避免空白高度
  const hasAnyValidData = hasValidTwitterData || hasTwitterLink || filteredAndSortedLinks.length > 0;
  if (!hasAnyValidData) {
    return null;
  }

  console.log('[ProjectInfo] ✅ Rendering with data:', {
    hasTwitter: hasValidTwitterData,
    hasTwitterLink: hasTwitterLink,
  ...
  return (
    <div className={styles.projectInfoContainer}>
      ...
      {hasTwitterLink && hasValidTwitterData && (
