...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  // 当模态框打开时，根据workflowForm中的cover初始化本地状态
  useEffect(() => {
    if (isOpen) {
      // 重置本地封面状态
      setCoverUrl(null)
      
      // 如果workflowForm中有cover，设置为本地状态
      if (workflowForm.cover) {
        setCoverUrl(workflowForm.cover)
      }
  ...
  }, [isOpen, workflowForm.cover])

  // 监听workflowForm.cover的变化，同步到本地状态
  useEffect(() => {
    if (isOpen && workflowForm.cover !== coverUrl) {
      setCoverUrl(workflowForm.cover || null)
      if (fileInputRef.current) {
        fileInputRef.current.value = ''
      }
    }
  }, [workflowForm.cover, isOpen, coverUrl])
  ...
  const handleFileUpload = async (file: File) => {
    ...
    // setCover(file) // Removed as per edit hint
                  ...
                  >
                    {isUploading ? (
                      ...
                      <>
                        ...
                        <span className="font-lexend font-normal text-xs leading-[100%] text-[#9CA3AF] text-center">
                          {isDragOver ? t('modal.dropImageHere') : t('workflow.uploadCover') || 'Click to upload or drag here'}
                        </span>
                      </>
                    )}
                    <input
