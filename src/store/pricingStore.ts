...
// 获取当前计划
export const getCurrentPlan = atom(
  (get) => {
...
// 检查是否为当前计划
export const isPlanCurrent = atom(
  (get) => (planId: string) => {
    const pricingState = get(pricingAtom);
...
// 订阅计划
export const subscribeToPlan = atom(
  null,
  async (get, set, planId: string) => {
    const pricingState = get(pricingAtom);
...
// 从账户状态同步角色/计划
export const syncPlanWithAccount = atom(
  null,
  (get, set) => {
